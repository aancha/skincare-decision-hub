...
Run `python3 examples/shortlist_ai/shortlist.py --scenario unknown-citation` to reproduce the validation rejection. Inspect the [five-minute code tour](code-tour.md), [paired evaluation design](../../examples/evaluation/README.md), and [verified live ML filter recipe](responsible-ml.md#try-the-bounded-live-comparison) next. The replay is published and its captured frames were checked; continuous-playback usability was explicitly deferred and unfamiliar-human validation remains pending. See [publication evidence and limits](evidence.md).
